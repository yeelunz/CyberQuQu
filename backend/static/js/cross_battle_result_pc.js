...
function renderCrossBattleResultPC(allCrossData) {
    ...
    function displayData(index) {
      ...
      // 清除之前的內容（除了選單）
      const existingSections = contentArea.querySelectorAll(".env-evaluation-section, .profession-evaluation-section, .combine-win-rate-section, .skill-usage-section, .data-info");
      ...
      // 渲染環境評估指標
      renderEnvEvaluation(crossData.data.env_evaluation, container = contentArea);
  
      // 渲染職業評估指標
      renderProfessionEvaluation(crossData.data.profession_evaluation, container = contentArea);
  
      // 渲染勝率對照表（Heat Map）
      renderCombineWinRateTable(crossData.data.combine_win_rate_table, container = contentArea);
  
      // 渲染技能使用頻率分析
      renderSkillUsageFrequency(crossData.data.profession_skill_used_freq, container = contentArea);
  ...
  function renderProfessionEvaluation(profession_evaluation, container) {
    ...
    // profession_evaluation 的結構是：
    // profession_evaluation[profName] = { "EIR": x, "Advanced NAS": y, "MSI": z, "PI20": w }
    for (const [profName, data] of Object.entries(profession_evaluation)) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td><img src="/static/images/${encodeURIComponent(profName)}.png" alt="${profName}" class="profession-avatar" /> ${profName}</td>
  ...
  function renderCombineWinRateTable(combine_win_rate_table, container) {
    ...
    professionList.forEach(prof => {
      combineThead += `<th><img src="/static/images/${encodeURIComponent(prof)}.png" alt="${prof}" class="profession-avatar-small" /><br>${prof}</th>`;
    });
    ...
    professionList.forEach(profA => {
      combineTbody += `<tr><th><img src="/static/images/${encodeURIComponent(profA)}.png" alt="${profA}" class="profession-avatar-small" /><br>${profA}</th>`;
      professionList.forEach(profB => {
        if (profA === profB) {
          combineTbody += `<td>—</td>`;
        } else {
          const matchData = combine_win_rate_table[profA] && combine_win_rate_table[profA][profB];
  ...
  function renderSkillUsageFrequency(skillusedFreq, container) {
    ...
    for (const [profession, skills] of Object.entries(skillusedFreq)) {
      // 計算該職業的總技能使用次數
      const totalSkillUsage = Object.values(skills).reduce((sum, freq) => sum + freq, 0);
  
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td><img src="/static/images/${encodeURIComponent(profession)}.png" alt="${profession}" class="profession-avatar" /> ${profession}</td>
        <td>
          <img src="/static/images/${encodeURIComponent(profession)}_skill_0.png" alt="Skill 0" class="skill-avatar-small" /><br>
          次數: ${skills[0] || 0} (${((skills[0] || 0) / totalSkillUsage * 100).toFixed(2)}%)
        </td>
        <td>
          <img src="/static/images/${encodeURIComponent(profession)}_skill_1.png" alt="Skill 1" class="skill-avatar-small" /><br>
          次數: ${skills[1] || 0} (${((skills[1] || 0) / totalSkillUsage * 100).toFixed(2)}%)
        </td>
        <td>
          <img src="/static/images/${encodeURIComponent(profession)}_skill_2.png" alt="Skill 2" class="skill-avatar-small" /><br>
    ...
    tables.forEach(table => {
      ...
      headers.forEach(header => {
        ...
        header.addEventListener("click", () => {
          ...
          rows.sort((a, b) => {
            const aValue = parseFloat(a.querySelector(`td:nth-child(${columnIndex})`).textContent);
            const bValue = parseFloat(b.querySelector(`td:nth-child(${columnIndex})`).textContent);
