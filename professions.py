...
class Paladin(BattleProfession):
    ...
    def apply_skill(self, skill_id, user, targets, env):
        ...
        if skill_id == 0:
            ...
            # 技能 2 => 恢復血量，第一次50, 第二次35, 第三次及以後10
...
class Assassin(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            name="刺客",
            base_hp=215,
            passive_desc="刺殺：攻擊時額外造成敵方當前5%生命值的傷害",
            baseAtk=1.25,
            baseDef=0.95
        )
    ...
    def apply_skill(self, skill_id, user, targets, env):
        ...
        if skill_id == 6:
            ...
            # 45% 1層 35% 2層 20% 3層
            add_stacks = random.choices([1, 2, 3], weights=[0.45, 0.35, 0.2], k=1)[0]
...
class Archer(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            name="弓箭手",
            base_hp=195,
            passive_desc="鷹眼：攻擊時15%機率造成2倍傷害；敵方防禦基值越高時，額外增加鷹眼的觸發機率。",
            baseAtk=1.15,
            baseDef=0.95
        )
...
class Berserker(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            name="狂戰士",
            base_hp=365,
            passive_desc="狂暴：若自身血量<50%時，攻擊增加失去生命值的30%的傷害。",
            baseAtk=1.0,
            baseDef=0.79
        )
    ...
    def apply_skill(self, skill_id, user, targets, env):
        ...
        if skill_id == 12:
            ...
            # 技能 13 => 消耗150點血量，接下來5回合每回合回復20點生命值。冷卻5回合。
            if user["hp"] > 150:
                env.deal_healing(user, 150,self_mutilation = True)
                heal_effect = HealthPointRecover(hp_recover=35, duration=5, stackable=False,source=skill_id,env=env)
...
class DragonGod(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            name="龍神",
            base_hp=200,
            passive_desc="龍血: 每回合疊加一個龍神狀態，龍神狀態每層增加3%攻擊力、3%防禦力",
            baseAtk=1.0,
            baseDef=1.0
        )
    ...
    def apply_skill(self, skill_id, user, targets, env):
        ...
        if skill_id == 15:
            ...
            if stacks > 0:
                ...
                env.set_status(user, "攻擊力" , resStacks,source = source)
                env.set_status(user, "防禦力" , resStacks,source = source)
                dragon_soul_effect.stacks = resStacks
...
class Ranger(BattleProfession):
    ...
    def apply_skill(self, skill_id, user, targets, env):
        ...
        if skill_id == 27:
            ...
            dmg = (35 + (20 * times_used)) * self.baseAtk 
...
class ElementalMage(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            name="元素法師",
            base_hp=190,
            passive_desc="元素之力：攻擊時 30% 造成(麻痺、冰凍、燃燒)其中之一",
            baseAtk=1.35,
            baseDef=0.95
        )
...
class HuangShen(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            baseAtk=1.15,
            baseDef=1.25,
            passive_desc="枯萎之刃：隨著造成傷害次數增加，攻擊時額外進行隨機追打，每造成兩次傷害增加一次最高追打機會，追打造成敵方當前生命的5%血量；額外追打不會累積傷害次數。",
        )
