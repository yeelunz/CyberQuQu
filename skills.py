...
sm.add_skill(Skill(1, "堅守防禦", "本回合迴避所有攻擊，並恢復 15 點生命值。冷卻 3 回合。", 'effect',3))
sm.add_skill(Skill(2, "神聖治療", "恢復血量：第一次恢復 50 點，第二次恢復 35 點，第三次及以後恢復 15 點。", 'heal'))
...
sm.add_skill(Skill(5, "全域爆破", "對敵方全體造成 25 點傷害，並引爆累積的狀態。每層燃燒或冰凍效果額外造成 40 點傷害。", 'damage'))

# 
sm.add_skill(Skill(6, "致命暗殺", "對單體造成 40 點傷害，30% 機率使傷害翻倍。", 'damage'))
sm.add_skill(Skill(7, "毒爆", "引爆目標的中毒狀態。每層中毒造成 15 點傷害，並為自身恢復 15 點生命值。", 'effect'))
sm.add_skill(Skill(8, "毒刃襲擊", "對單體造成 10 點傷害，並疊加 1~3 層中毒狀態（最多 5 層）。每層中毒狀態造成 3 點傷害。", 'damage'))

# 
sm.add_skill(Skill(9, "五連矢", "對單體造成 50 點傷害，並使其防禦力降低 10%。", 'damage'))
sm.add_skill(Skill(10, "箭矢補充", "2 回合間，增加 250% 攻擊力，或降低自身防禦力 50%。", 'effect'))
...
# 
sm.add_skill(Skill(15, "神龍之息", "對單體造成 30 點傷害，每層龍神狀態增加 5 點額外傷害。", 'damage'))
sm.add_skill(Skill(16, "龍血之淵", "恢復 100 點生命值，接下來 3 回合，每回合扣除 30 點生命值。冷卻 4 回合。", 'heal',4))
sm.add_skill(Skill(17, "神龍燎原", "消除一半的龍神層數，對敵方造成每層 35 點傷害。", 'damage'))

# 血神
sm.add_skill(Skill(18, "血刀", "對單體造成 45 點傷害。", 'damage'))
sm.add_skill(Skill(19, "飲血", "消耗敵方一半的流血層數。每層流血造成 10 點傷害，並為自身恢復 10 點生命值。", 'damage'))
...
sm.add_skill(Skill(23, "絕地反擊", "對攻擊者立即造成其本次攻擊傷害的 300%。此技能需冷卻 3 回合。", 'damage',3))
...
sm.add_skill(Skill(26, "堅硬皮膚", "提升 35% 防禦力，持續3回合。", 'effect'))

# Ranger (荒原遊俠) 技能定義
sm.add_skill(Skill(27, "續戰攻擊", "造成 35 點傷害，每次連續使用攻擊技能時額外增加 15 點傷害。", 'damage'))
sm.add_skill(Skill(28, "埋伏防禦", " 3 回合內，提升 25 % 防禦力。", 'effect'))
sm.add_skill(Skill(29, "荒原抗性", "消耗 15 點生命力，免疫一回合的傷害。", 'effect'))

# ElementalMage (元素法師) 技能定義
sm.add_skill(Skill(30, "雷霆護甲", "2 回合內，受到傷害時有 30% 機率直接麻痺敵人，並增加 50% 防禦力，回復最大生命的 5% ", 'effect'))
sm.add_skill(Skill(31, "凍燒雷", "造成 55 點傷害，對每層麻痺、冰凍、燃燒狀態額外造成 20 點傷害。", 'damage'))
sm.add_skill(Skill(32, "雷擊術", "造成 25 點傷害，35 % 機率使敵方暈眩 2~4 回合。", 'effect'))
...
#  星神
sm.add_skill(Skill(36, "光輝流星", "對敵方單體造成 25 點傷害，並隨機為自身附加以下一種增益效果，持續 3 回合：攻擊力提升 5%，防禦力提升 5%，治癒效果提升 5%。", 'damage'))
sm.add_skill(Skill(37, "災厄隕星", "為自身恢復 25 點生命值，並隨機為敵方附加以下一種減益效果，持續 3 回合：攻擊力降低 5%，防禦力降低 5%，治癒效果降低 5%。", 'damage'))
sm.add_skill(Skill(38, "虛擬創星圖", "對敵方單體造成 50 點傷害，本回合天啟星盤的效果增加 50% 。", 'damage'))
