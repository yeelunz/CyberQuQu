...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderManageModelsPage(models) {
      ...
      models.forEach((modelObj) => {
        ...
        if (!meta.elo_result) { // 修改此行，避免 `null` 或 `undefined`
          // 顯示「無資訊」和「計算 ELO」按鈕
    ...
    function startComputeELO(folderName, btn, detailsDiv) {
          ...
          switch (data.type) {
            ...
            case "done":
              ...
              if (data.new_elo_result) {
                ...
                // 生成職業詳細 ELO 卡片
                const eloDetailsHTML = generateEloDetailsHTML(data.new_elo_result["詳細"]);
            ...
            case "stopped":
              ...
              stoppedMsg.textContent = data.message || "ELO 計算已被終止。";
            ...
            case "error":
              ...
              errorMsg.textContent = data.message || "ELO 計算過程中發生錯誤。";
      ...
      source.onerror = (err) => {
        ...
        if (!completedNormally) {
          ...
          const errorMsg = document.createElement("div");
          errorMsg.style.color = "red";
          errorMsg.textContent = "ELO 計算過程中連線中斷或發生錯誤。";
          detailsDiv.appendChild(errorMsg);
          btn.disabled = false;
        }
      };
    ...
    function generateEloTotalHTML(totalElo) {
      ...
      return `
        <div class="mm-elo-total">
          ...
          <p><strong>總和先攻方ELO:</strong> ${Math.round(totalElo["總和先攻方ELO"])}</p>
          <p><strong>總和後攻方ELO:</strong> ${Math.round(totalElo["總和後攻方ELO"])}</p>
