...
class GodOfStar(BattleProfession):
    def __init__(self):
        super().__init__(
            ...
            passive_name="天啟星盤",
            passive_desc=f"星神在戰鬥中精通增益與減益效果的能量運用。每當場上有一層「能力值增益」或「減益」效果時，每回合會額外對敵方造成 {GODOFSTAR_VAR['GODOFSTAR_PASSIVE_DAMAGE_PER_EFFECT'][0]} 點傷害 並恢復 {GODOFSTAR_VAR['GODOFSTAR_PASSIVE_HEAL_PER_EFFECT'][0]} 點生命值。"
        )
